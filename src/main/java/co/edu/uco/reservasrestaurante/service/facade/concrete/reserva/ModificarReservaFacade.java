...
import co.edu.uco.reservasrestaurante.service.businesslogic.concrete.reserva.ModificarReservaUseCase;
import co.edu.uco.reservasrestaurante.service.businesslogic.validator.concrete.reserva.ModificarReservaValidator;
...
public final class ModificarReservaFacade implements Facade<ReservaDTO>{

	@Override
	public void execute(ReservaDTO dto) {
		final ReservaDomain domain = ReservaDTOMapper.convertToDomain(dto);
		ModificarReservaValidator.ejecutar(domain);
		...
		try {
			...
			var useCase = new ModificarReservaUseCase(daoFactory);
			...
			var mensajeUsuario = CatalogoMensajes.obtenerContenidoMensaje(CodigoMensaje.M00000254);
			var mensajeTecnico = CatalogoMensajes.obtenerContenidoMensaje(CodigoMensaje.M00000255);
